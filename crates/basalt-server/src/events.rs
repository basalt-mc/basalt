...
/// Implements [`Event`] for a cancellable event struct (has `cancelled: bool` field).
...
/// Implements [`Event`] for a non-cancellable event struct.
macro_rules! event {
    ($name:ident) => {
        impl Event for $name {
            fn is_cancelled(&self) -> bool {
...
///
/// Fired after the player's position has been updated in `PlayerState`.
/// Carries the previous chunk coordinates for chunk boundary detection.
/// Not cancellable — the server is not authoritative for position.
