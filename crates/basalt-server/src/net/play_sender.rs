...
pub(super) async fn write_server_output(
    ...
    match output {
        ...
        ServerOutput::RecipeBookAdd { entries, replace } => {
            ...
            let packet = ClientboundPlayRecipeBookAdd {
                entries: entries.clone(),
                replace: *replace,
            };
            conn.write_packet_typed(ClientboundPlayRecipeBookAdd::PACKET_ID, &packet)
                .await?;
        }
        ServerOutput::RecipeBookRemove { display_ids } => {
            use basalt_protocol::packets::play::misc::ClientboundPlayRecipeBookRemove;
            let packet = ClientboundPlayRecipeBookRemove {
                recipe_ids: display_ids.clone(),
            };
            conn.write_packet_typed(ClientboundPlayRecipeBookRemove::PACKET_ID, &packet)
                .await?;
        }
        ServerOutput::SendGhostRecipe { window_id, display } => {
            ...
            let packet = ClientboundPlayCraftRecipeResponse {
                window_id: *window_id,
                recipe_display: display.clone(),
            };
            conn.write_packet_typed(ClientboundPlayCraftRecipeResponse::PACKET_ID, &packet)
                .await?;
        }
