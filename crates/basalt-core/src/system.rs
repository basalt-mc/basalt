...
pub trait SystemContextExt {
    ...
    fn get<T: crate::Component>(&self, entity: EntityId) -> Option<&T>;

    /// Returns a typed mutable component reference.
    fn get_mut<T: crate::Component>(&mut self, entity: EntityId) -> Option<&mut T>;

    /// Sets a typed component value for an entity.
    fn set<T: crate::Component>(&mut self, entity: EntityId, component: T);

    /// Returns all entity IDs that have a component of type `T`.
    fn query<T: crate::Component>(&self) -> Vec<EntityId>;
}
