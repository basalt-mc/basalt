...
use crate::types::{PacketDef, SwitchEnum};
...
pub(crate) fn generate_state_module(
    ...
    let serverbound = parse_direction(
        state,
        "toServer",
        "Serverbound",
        &pascal_state,
    ...
    let clientbound = parse_direction(
        state,
        "toClient",
        "Clientbound",
        &pascal_state,
...
pub(crate) fn generate_imports(packets: &[&PacketDef]) -> String {
    ...
    let has_switch_enums = packets.iter().any(|p| !p.switch_enums.is_empty());
    ...
    if has_inline_structs || has_switch_enums {
...
pub(crate) fn generate_packet_struct(packet: &PacketDef) -> String {
    ...
    // Emit switch enums before the packet struct
    for switch_enum in &packet.switch_enums {
        out.push_str(&generate_switch_enum(switch_enum, &packet.name));
        out.push('\n');
    }
...
/// Generates an enum from a group of switch fields.
///
/// The enum has one variant per discriminator value. Each variant
/// contains the fields that are present for that value, with
/// appropriate `#[field(...)]` and `#[variant(id = N)]` attributes.
fn generate_switch_enum(switch_enum: &SwitchEnum, packet_name: &str) -> String {
    let mut out = String::new();
    ...
    for variant in &switch_enum.variants {
    ...
    // Generate a Default impl that returns the first variant.
    // Required because packet structs derive Default.
    let first = &switch_enum.variants[0];
    out.push_str(&format!(
        "impl Default for {} {{\n    fn default() -> Self {{\n        Self::{}",
        switch_enum.name, first.variant_name
    ));
